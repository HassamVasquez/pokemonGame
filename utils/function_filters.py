...
def filter_type(list_type):
    def func_pokemon(pokemon: list[Pokemon]):
        match len(list_type):
            case 1:
                if(pokemon.type_1 in list_type or pokemon.type_2 in list_type): return True
                else : return False
            case 2:
...
def filter_gen(list_gen):
    def func_pokemon(pokemon):
        if(str(pokemon.generation) in list_gen or not list_gen): return True
        else : return False
    return func_pokemon
    
def list_filtred(list_pokemons, dict_filters):
    if dict_filters['type'] or dict_filters['gen']:
        f_type, f_gen = filter_type(dict_filters['type']), filter_gen(dict_filters['gen'])
